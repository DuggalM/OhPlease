...
class PeakOffpeak(object):
    ...
    def run(self, trips_hhold_df, peak_consistency):
        ...
        np.random.seed(control_parameters.seed)
        ...
        control_parameters.logger.info("Return the trips and household dataframe combined with each record that has a home end"
                                 "in it tagged as to whether it starts in the peak (1) or off-peak period (0).")
        ...
        if peak_consistency == 1:
            control_parameters.logger.info("Peak consistency flag set to %s." % peak_consistency)
            ...
            trips_hhold_df_array = trips_hhold_df_array.astype(dtype= {
        ...
        control_parameters.logger.info("Peak flag populated")
