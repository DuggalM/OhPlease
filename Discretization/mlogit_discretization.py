...
class ModeSampling(object):
    ...
    def vehtype_prob(self, purp_prob, veh_segment):
        ...
        if val % 2 == 0:
            ...
            control_parameters.logger.error('The columns are not even numbered. The slicing of the purpose dataframe is wrong')
    ...
    def elemental_mode(self, mand_prob_l1, veh_segment, mand_prob_append, purpose, trips):
        ...
        if len(mand_level2) >0:
            control_parameters.logger.info("Start sampling of the elemental mode for vehicle segment %s. This is initiated "
                               "provided the binary probability file has records with "
                               "non-zero probabilities. A total of %s records are in the df" %(veh_segment, len(mand_level2)))
            # sample using Cheval
            sampled_df = pd.DataFrame(sample_from_weights(mand_level2, randomizer=self.prng,
        ...
        if len(collect_df) > 0:
            control_parameters.logger.info("Concatenate the dictionary of dataframes by vehicle segment")
            ...
            mand_mode['PrimaryMode'] = mand_mode['PrimaryMode'].astype('category')

            return mand_mode

        # return empty df
        mand_mode = pd.DataFrame()
        control_parameters.logger.info("Returning an empty dataframe because there were no elemental probabilities in the "
                           "i-j pairs for the vehicle segment %s " % veh_segment)
        return mand_mode


    def access_station_df(self, mand_st_acc, trips_vehtype):

        trips_vehtype = trips_vehtype[['taz_i', 'taz_j']]
        trips_vehtype.columns = ['Production Zone', 'Destination Zone']
        mand_st_acc = mand_st_acc.merge(trips_vehtype.drop_duplicates(), on=['Production Zone','Destination Zone'],
             how='left', indicator=True)
        mand_st_acc = mand_st_acc[mand_st_acc['_merge'] == "both"].copy()
        mand_st_acc.drop('_merge', axis=1, inplace = True)
        ...
        return (mand_st_acc_unstack)


    def egress_station_df(self, mand_st_egg, trips_vehtype):

        trips_vehtype = trips_vehtype[['taz_i', 'taz_j']]
        trips_vehtype.columns = ['Production Zone', 'Destination Zone']
        mand_st_egg = mand_st_egg.merge(trips_vehtype.drop_duplicates(), on=['Production Zone','Destination Zone'],
                   how='left', indicator=True)
        mand_st_egg = mand_st_egg[mand_st_egg['_merge'] == "both"].copy()
        mand_st_egg.drop('_merge', axis=1, inplace=True)
        ...
        return (mand_st_egg_unstack)


    def access_egress_station(self,  mand_mode, mand_st_acc_unstack, mand_st_egg_unstack):
        """
    ...
    def egress_prob_df(self, mand_eg_prob, trips_vehtype):
        ...
        trips_vehtype = trips_vehtype[['taz_i', 'taz_j']]
        trips_vehtype.columns = ['Production Zone', 'Destination Zone']
        mand_eg_prob = mand_eg_prob.merge(trips_vehtype.drop_duplicates(), on=['Production Zone','Destination Zone'],
                   how='left', indicator=True)
        mand_eg_prob = mand_eg_prob[mand_eg_prob['_merge'] == "both"].copy()
        mand_eg_prob.drop('_merge', axis=1, inplace=True)
    ...
    def egress_prob(self, mand_mode, melt_df):
        ...
        # ' sample egress mode
        sampled_df_eg = pd.DataFrame(sample_from_weights(egg_df_gr1, randomizer=self.prng,
    ...
    def getPeak_OffPeakFile(self, peak_offpeak, purpose):
        ...
        if peak_offpeak.upper() == 'PEAK':
            ...
            control_parameters.logger.error("Invalid time period definition")
    ...
    def run(self, peak_offpeak, purpose, trips_vehtype):
        ...
        trips_vehtype = trips_vehtype[trips_vehtype['purpose'] == purpose].copy()

        # batch requisite json file for DTYPE
        control_parameters.logger.info("Batch in the DTYPE definitions")
        ...
        for chunk_prob, chunk_station, chunk_egress in zip(hbw_trip_bin, hbw_stn_bin, hbw_egg_bin):
            ...
            hbw_prob = pd.DataFrame(np.fromfile(os.path.join(control_parameters.dirListing_mlogit_prob, chunk_prob),
                                                dtype=[tuple(t) for t in
                                                       dataFrameDtype[ev.EarlyValidFiles.DTYPE_ELEMENTAL_PROB]]))

            # need to set Bill's market segmwents to start from 0
            hbw_prob['Market Segment'] = hbw_prob['Market Segment'] - 1

            hbw_stn = pd.DataFrame(np.fromfile(os.path.join(control_parameters.dirListing_mlogit_prob, chunk_station),
                                               dtype=[tuple(t) for t in
                                                      dataFrameDtype[ev.EarlyValidFiles.DTYPE_STATION_CHOICE]]))

            hbw_eg_prob = pd.DataFrame(np.fromfile(os.path.join(control_parameters.dirListing_mlogit_prob, chunk_egress),
                                                   dtype=[tuple(t) for t in
                                                          dataFrameDtype[ev.EarlyValidFiles.DTYPE_EGRESS_PROB]]))
            ...
            # common.logger.info("A total of %  O-D pairs are to be processed." % len(hbw_prob_l1))
            ...
            control_parameters.logger.info("Create access/egress stations and egress probability dataframe")
            mand_st_acc_unstack = self.access_station_df(hbw_st_acc, trips_vehtype)
            mand_st_egg_unstack = self.egress_station_df(hbw_st_egg, trips_vehtype)
            melt_df = self.egress_prob_df(hbw_eg_prob, trips_vehtype)

            control_parameters.logger.info("Start running vehicle segments for each chunk %s" % chunk_prob)
            ...
            for veh_segment in range(0, 4):
                ...
                if len(hbw_mode) > 0:
                    ...
                    hbw_mode = self.access_egress_station(hbw_mode,mand_st_acc_unstack, mand_st_egg_unstack)
                    hbw_mode = self.egress_prob(hbw_mode, melt_df)
