...
class NonMandatoryFortranOd(object):
    ...
    def __init__(self, all_other):
        self.all_other = all_other



    def sample_destination(self, current_row, chaos_monkey, prng):
        """
        ...
        t1 = all_other[ev.EarlyValidFiles.dict_othertrips_names.get(current_row[23])]
    ...
    def destination_solver(self, trips_hhold_array, chaos_monkey):
        ...
        for row in range(nrows):
            if (row % LOG_EVERY_N) == 0:
                control_parameters.logger.info("Solving household number %s." % trips_hhold_array[row][0])
            ...
            if (trips_hhold_array[row][7] > 0) & (trips_hhold_array[row][8] == 0):
                ...
                new_taz_j = self.sample_destination(trips_hhold_array[row], chaos_monkey, prng)
            ...
            if (trips_hhold_array[row][7] == 0) & (trips_hhold_array[row][8] == 0):
                ...
                new_taz_j = self.sample_destination(test_current, chaos_monkey, prng)
                trips_hhold_array[row][8] = new_taz_j

        return trips_hhold_array


    def slice_dataframe_forparallel(self, trips_hhold, cpu_cores):

        trips_hhold['parallel_flag'] = trips_hhold.groupby('hhid').ngroup()
        total = max(trips_hhold['parallel_flag'])
        start_pos = 0
        end_pos = 0
        ...
        if total % cpu_cores != 0:
            batch_size = int(total / cpu_cores)
            print("Batch size is %s" % batch_size)
        else:
            batch_size = total / cpu_cores
            print("Batch size is %s" % batch_size)
        ...
        for i in range(cpu_cores):
            #     print(i)
            if i < cpu_cores - 1:
                start_pos = i * batch_size + add
                end_pos = i * batch_size + batch_size
                add = 1
            ...
            df_sl = trips_hhold[(trips_hhold['parallel_flag'] >= start_pos) & (trips_hhold['parallel_flag'] <= end_pos)]
            df_sl.drop('parallel_flag', axis=1)
            grp_list.append(df_sl)
    ...
    def reorder_setdtype(self, trips_hhold):
    ...
    def save_matrices(self, nonmandatory_purposes, trips_hhold, ggh2):
